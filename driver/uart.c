    ...
    pRxBuff->BuffState = UNDER_WRITE;
        ...
        *(pRxBuff->pWritePos) = RcvChar;
        ...
        if (pRxBuff->BuffState == WRITE_OVER){
        	 ...
        	 os_memset(pRxBuff->pWritePos, 0 , 64);
			 pRxBuff->pWritePos= pRxBuff->pWritePos-msg_count;
			 os_memcpy(msgBuffer,pRxBuff->pWritePos, 64);

			 ets_uart_printf("message: %s \n", msgBuffer);

			 os_timer_disarm(&uartTimer);

			 if (uartCbp == NULL)
				 os_timer_setfn(&uartTimer,(ETSTimerFunc *) null_function , (void *) msgBuffer);
			 else
				 os_timer_setfn(&uartTimer,(ETSTimerFunc *) uartCbp , (void *) msgBuffer);

			 os_timer_arm(&uartTimer , 100 , 0);
